...
public final class CameraViewController: UIViewController {
  ...
  private var frontCameraDevice: AVCaptureDevice.DiscoverySession? {

    return AVCaptureDevice.DiscoverySession(deviceTypes: [AVCaptureDevice.DeviceType.builtInTrueDepthCamera], mediaType: AVMediaType.video, position: .front)
...
//    return AVCaptureDevice.devices(for: .video).first(where: { $0.position == .front })
  ...
  private func setupSessionInput(for position: AVCaptureDevice.Position) {
    ...
    do {
        let newInput = try AVCaptureDeviceInput(device: device as! AVCaptureDevice)
...
private extension CameraViewController {
  ...
  func makeSettingsButton() -> UIButton {
    ...
    let title = NSAttributedString(
      string: localizedString("Scan Barcode"),
